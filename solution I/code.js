...
function display(page_id, page_size) {
  ...
  for (i = lower_bound; i < data.results.length && i < higher_bound; i++) {
    ...
    // wdith= 100 * data.results[i].vote_average/10 ;
    count = 0;
    if (i % 3 == 0)
      to_add += `<div id="img_group">`
    to_add += `<div id="img_container" > 
  ...
  if (higher_bound <= 3)
    jQuery("#img_group").css("flex-direction", "row")
  // console.log(higher_bound);
